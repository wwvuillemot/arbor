...
describe('Configuration Page Translations', () => {
  it('should have translations for Configuration section', () => {
    ...
    expect(messages.settings.configuration.description).toContain('connection URLs');
  });

  it('should have translations for Database URL', () => {
    expect(messages.settings.configuration.databaseUrl.label).toBe('Database URL');
    expect(messages.settings.configuration.databaseUrl.placeholder).toBe('postgres://arbor:arbor@localhost:5432/arbor');
    expect(messages.settings.configuration.databaseUrl.description).toContain('PostgreSQL');
  });

  it('should have translations for Redis URL', () => {
    expect(messages.settings.configuration.redisUrl.label).toBe('Redis URL');
    expect(messages.settings.configuration.redisUrl.placeholder).toBe('redis://localhost:6379');
    expect(messages.settings.configuration.redisUrl.description).toContain('Redis');
  });

  it('should have translations for API URL', () => {
    expect(messages.settings.configuration.apiUrl.label).toBe('API URL');
    expect(messages.settings.configuration.apiUrl.placeholder).toBe('http://localhost:3001');
    expect(messages.settings.configuration.apiUrl.description).toContain('API');
  });

  it('should have translations for Ollama Base URL', () => {
    expect(messages.settings.configuration.ollamaBaseUrl.label).toBe('Ollama Base URL');
    expect(messages.settings.configuration.ollamaBaseUrl.placeholder).toBe('http://localhost:11434');
    expect(messages.settings.configuration.ollamaBaseUrl.description).toContain('Ollama');
  });
