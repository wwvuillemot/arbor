...
describe('SettingsService', () => {
  ...
  describe('setSetting', () => {
    it('should encrypt and store a setting', async () => {
      await settingsService.setSetting('openai_api_key', 'sk-test-key-123', testMasterKey);
    ...
    it('should update existing setting', async () => {
      await settingsService.setSetting('openai_api_key', 'old-key', testMasterKey);
      await settingsService.setSetting('openai_api_key', 'new-key', testMasterKey);

      const decrypted = await settingsService.getSetting('openai_api_key', testMasterKey);
  ...
  describe('getSetting', () => {
    it('should decrypt and return a setting', async () => {
      ...
      const value = await settingsService.getSetting('openai_api_key', testMasterKey);
      
      expect(value).toBe('sk-test-key-123');
    });

    it('should return null for non-existent setting', async () => {
      const value = await settingsService.getSetting('nonexistent', testMasterKey);
  ...
  describe('getAllSettings', () => {
    it('should return all settings decrypted', async () => {
      await settingsService.setSetting('openai_api_key', 'sk-openai-123', testMasterKey);
      await settingsService.setSetting('anthropic_api_key', 'sk-anthropic-456', testMasterKey);
    ...
    it('should throw error if any setting fails to decrypt', async () => {
      ...
      const wrongKey = 'YmJiYmJiYmJiYmJiYmJiYmJiYmJiYmJiYmJiYmJiYmI=';
      await expect(
        settingsService.getAllSettings(wrongKey)
      ).rejects.toThrow();
    });
  });
