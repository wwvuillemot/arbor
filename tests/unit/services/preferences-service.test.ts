...
describe('PreferencesService', () => {
  ...
  describe('Session-scope preferences (Redis)', () => {
    ...
    it('should set and get a session preference', async () => {
      await preferencesService.setSessionPreference(sessionId, 'tempView', 'grid');
      const value = await preferencesService.getSessionPreference(sessionId, 'tempView');

      expect(value).toBe('grid');
    });

    it('should return null for non-existent session preference', async () => {
      const value = await preferencesService.getSessionPreference(sessionId, 'nonexistent');
    ...
    it('should store different values for different sessions', async () => {
      ...
      const value1 = await preferencesService.getSessionPreference(session1, 'view');
      const value2 = await preferencesService.getSessionPreference(session2, 'view');
    ...
    it('should store complex objects in session', async () => {
      ...
      await preferencesService.setSessionPreference(sessionId, 'viewState', complexValue);
      const value = await preferencesService.getSessionPreference(sessionId, 'viewState');
    ...
    it('should respect custom TTL', async () => {
      ...
      await preferencesService.setSessionPreference(sessionId, 'shortLived', 'value', 1);

      // Should exist immediately
      let value = await preferencesService.getSessionPreference(sessionId, 'shortLived');
      ...
      // Should be expired
      value = await preferencesService.getSessionPreference(sessionId, 'shortLived');
      expect(value).toBeNull();
    }, 5000);
  });
});
