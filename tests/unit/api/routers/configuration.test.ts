...
describe('Configuration Router', () => {
  ...
  describe('getConfiguration', () => {
    it('should return default value when not set', async () => {
      const result = await caller.configuration.getConfiguration({ key: 'DATABASE_URL' });
    ...
    it('should work for all configuration keys', async () => {
      const databaseUrl = await caller.configuration.getConfiguration({ key: 'DATABASE_URL' });
      const redisUrl = await caller.configuration.getConfiguration({ key: 'REDIS_URL' });
      const apiUrl = await caller.configuration.getConfiguration({ key: 'API_URL' });
      const ollamaUrl = await caller.configuration.getConfiguration({ key: 'OLLAMA_BASE_URL' });
  ...
  describe('resetConfiguration', () => {
    it('should reset configuration to default', async () => {
      ...
      // Verify it's set
      let result = await caller.configuration.getConfiguration({ key: 'DATABASE_URL' });
      expect(result).toBe('postgres://custom:custom@custom:5432/custom');
      ...
      result = await caller.configuration.getConfiguration({ key: 'DATABASE_URL' });
      expect(result).toBe(DEFAULT_CONFIG.DATABASE_URL);
    });
  });
  ...
  describe('isCustomized', () => {
    it('should return false for default values', async () => {
      const result = await caller.configuration.isCustomized({ key: 'DATABASE_URL' });
